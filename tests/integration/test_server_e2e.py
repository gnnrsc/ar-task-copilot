...
# Detect whether Google Application Default Credentials are available.
# Tests that require real GCP credentials (e.g. Vertex AI Agent Engine) are
# skipped automatically when running locally without ADC configured.
try:
    import google.auth
    google.auth.default()
    _HAS_ADC = True
...
def test_adk_run_sse(server_fixture: subprocess.Popen[str]) -> None:
    ...
    has_output = any(event.get("output") for event in events)
    assert has_text_content or has_output, "Expected at least one event with text or structured output"
...
@pytest.mark.skipif(
    not _HAS_ADC,
    reason="Requires Google Application Default Credentials (ADC) — run `gcloud auth application-default login` to enable.",
)
