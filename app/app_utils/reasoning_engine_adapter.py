...
def attach_reasoning_engine_routes(app: FastAPI) -> None:
    ...
    def get_runtime() -> AdkApp:
        ...
        if runtime is None:
            ...
            # Ensure a project is registered in the Vertex AI global config so
            # AdkApp.__init__ doesn't trigger an ADC lookup when reading
            # `initializer.global_config.project`. In production the project is
            # provided via gcloud / GOOGLE_CLOUD_PROJECT; locally we fall back
            # to a dummy value so tests pass without GCP credentials.
            project = os.environ.get("GOOGLE_CLOUD_PROJECT", "dummy-project")
            try:
                if aiplatform.initializer.global_config._project is None:
                    aiplatform.initializer.global_config._project = project
            except Exception:
                pass
